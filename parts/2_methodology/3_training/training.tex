...
We proceeded to do the same experiment as Garcia-Ceja, E. et al., as it would be a good way of comparing our work. For each participant, we generated input data that did not contain any activity data from the participant. Then we created a model and trained it to fit the input data for ten epochs (any more would take too much time), and made predictions using majority voting. 

\begin{figure}[h]
\begin{center}
      \includegraphics[height=8cm]{img/control_condition/leave_one_out.pdf}
      \caption{\textit{Confusion matrix containing detected classes after leave one participant out experiment. The model was able to classify true negatives (control group) better than anything else.}}
      \label{figure:confusion_matrix_leave_one_out}
\end{center}
\end{figure}

Earlier results were almost perfect, so we expected the results of this experiment to be better than what we can see above. Detecting that a participant belongs to the condition group was not that 

It is interesting that the model detects non-depressed participants better than depressed. 
