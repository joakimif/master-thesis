...
The dataset is in two parts. One part includes sensor data about the condition group and control group, as one file for each person. 
...
As stated in my problem statement, I want to perform different types of machine learning to do predictions on bipolar patients. Specifically, I want to do
predictions on their MADRS score. % more

To do this, I will use all the algorithms I have specified, with both regression and classification, beginning with the least complicated ones 
(decision tree learning and random forest), before I will do predictions using a neural network (LSTM RNN), and compare them. The next part of the 
thesis will cover more specifics on how I will use these machine learning models to acheive my goal. % remember this
...
However, in this project all data is anonymized (only referenced by an id), so there will be no persons mentioned. 
